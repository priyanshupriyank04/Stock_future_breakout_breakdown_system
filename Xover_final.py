...
def calculate_cboe_for_table(table_name: str, smoothK=3, smoothD=3, lengthRSI=14, lengthStoch=14, lengthcboe=5):
    """
...
def run_relative_strength_strategy():
    """
    Strategy 1 (latest-candle relative crossovers ONLY):
    - Bearish Trigger (must happen on the latest candle only):
        odd_bear crosses ABOVE odd_bull (idx=2 vs idx=1)
        AND di_minus crosses ABOVE di_plus (idx=2 vs idx=1)

    - Bullish Trigger (must happen on the latest candle only):
        odd_bull crosses ABOVE odd_bear (idx=2 vs idx=1)
        AND di_plus crosses ABOVE di_minus (idx=2 vs idx=1)

    Saves:
        - final_bear.csv  (bearish triggers, one contract per row)
        - final_bull.csv  (bullish triggers, one contract per row)
    """
    conn = connect_to_db()
    if not conn:
    ...
    for (tradingsymbol,) in futures_list:
        ...
        try:
            ...
            # Pick indices for latest crossover check
            # If 3 rows available, use (1 -> 2). If only 2 rows, it's (0 -> 1).
            if len(df) >= 3:
                prev_idx, curr_idx = 1, 2
            else:
                prev_idx, curr_idx = 0, 1

            # Helper for detecting crossover at the selected (prev->curr)
            def crossed_above(series1, series2, prev_i, curr_i):
                return (series1.iloc[prev_i] <= series2.iloc[prev_i]) and (series1.iloc[curr_i] > series2.iloc[curr_i])

            # Bearish: odd_bear ↑ over odd_bull AND di_minus ↑ over di_plus on the latest candle
            bear_cboe_cross_latest = crossed_above(df["odd_bear"], df["odd_bull"], prev_idx, curr_idx)
            bear_di_cross_latest   = crossed_above(df["di_minus"], df["di_plus"], prev_idx, curr_idx)

            # Bullish: odd_bull ↑ over odd_bear AND di_plus ↑ over di_minus on the latest candle
    ...
    # Save results to CSVs (same filenames for continuity)
    with open("final_bear.csv", "w", newline="") as f_bear:
        for symbol in bear_triggers:
            f_bear.write(symbol + "\n")

    with open("final_bull.csv", "w", newline="") as f_bull:
        for symbol in bull_triggers:
            f_bull.write(symbol + "\n")

    logging.info(
        f"Strategy relative crossover (latest-candle only, no Stoch) completed. "
